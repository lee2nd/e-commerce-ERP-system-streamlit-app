...
1. **`utils/parsers.py` — 新增 `parse_mo()` 解析函式**
   - 欄位對應：`訂單編號`、`轉單日`→`日期`、`商品名稱`、`商品原廠編號`→`貨號`、`數量`、`商品售價`→`單價`
   - 訂單狀態對應：配送結束→已完成、已回收→退貨、配送異常→未取貨、銷退原因=配送異常結案→未取貨、取消訂單→已取消

2. **`utils/calculators.py` — 新增 `_process_mo()` 日報表計算函式**
   - 折扣優惠 = 單品折價券(商品自折) ＋ 行銷活動促銷(商品自折) ＋ 單店抵用券(商品自折)
   - 買家支付運費 = 客人支付運費；平台補助運費 = 商品滿額免運費
   - 實際運費支出 = 預估平台代扣運費(鑑賞期後:平台代扣運費)
   - 物流處理費（運費差額）= 買家支付運費 − 實際運費支出
   - 未取貨/退貨運費（訂單狀態為已回收或配送異常時）= 活動服務費 ＋ 訂單進帳金額
   - 成交手續費 = 成交手續費；金流與系統處理費 = 金流與系統處理費；發票處理費 = 發票處理費
   - 其他服務費 = 預購商品服務費 ＋ 物流隱碼服務費 ＋ 活動服務費
   - 退貨/未取貨訂單：折扣優惠、商品成本、手續費、其他服務費等歸零，只保留未取貨/退貨運費
   - `generate_daily_report()` 新增 `mo_raw` 參數

3. **`utils/data_manager.py`** — `MO店.xlsx` 納入快取清除與一鍵刪除全部資料範圍

4. **`app.py`** — MO店 納入備份 / 還原的檔案清單；首頁副標題顯示 MO店

5. **`pages/1_📥_匯入資料.py`** — 支援 MO店 Excel 上傳（`parse_mo`）、累積訂單顯示、清除訂單

6. **`pages/2_📋_對照表.py`** — MO店 納入平台掃描、排序（第 3 順位）、色彩標示（紫色 `#AB63FA`）

7. **`pages/3_📦_導出出庫.py`** — `_filter_mo()` 過濾取消/退貨/未取貨；MO店 統計欄位、色彩標示
...
### MO店
| 情境（訂單狀態）| 日報表訂單狀態 | 庫存 / 出庫 |
|----------------|---------------|-------------|
| 配送結束 | 已完成 | 扣庫存、計入出庫 |
| 已回收 | 退貨；`未取貨/退貨運費` = 活動服務費 ＋ 訂單進帳金額 | 不扣庫存、不計入出庫 |
| 配送異常 | 未取貨；`未取貨/退貨運費` = 活動服務費 ＋ 訂單進帳金額 | 不扣庫存、不計入出庫 |
| 銷退原因 = 配送異常結案 | 未取貨（同上） | 不扣庫存、不計入出庫 |
| 取消訂單 | 不列出 | 不扣庫存、不計入出庫 |
